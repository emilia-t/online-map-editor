...
export default new Vuex.Store({
  state: {
    constant: {},
    classList: {
      realisticTile: class realisticTile {
        //真实地图底图类
        constructor(mountDocument, baseMapOptions) {

          console.log(mountDocument);
          this.ctx = null;
          this.el = null; //挂载点
          this.tileSize = 256;
          this.network = {
        ...
        initial(mountDocument, baseMapOptions) {
          //初始化
          console.log(mountDocument);
          console.log(baseMapOptions);
          console.log("????");
          ...
          let latLng = this.latLngRotate(
            this.options.defaultY,
            this.options.defaultX
          );
          if (latLng === false) return false;
          this.view.offsetY =
            this.map.height / 2 -
            this.latToTileY(latLng.lat, this.options.defaultZoom) *
              this.tileSize;
          this.view.offsetX =
            this.map.width / 2 -
            this.lngToTileX(latLng.lng, this.options.defaultZoom) *
              this.tileSize;
          this.render();
        }
        render() {
          ...
          let rightBottomPos = this.screenToTile(
            this.map.width,
            this.map.height
          );
          let minNumberX = this.callTileNumber(
            leftTopPos.x,
            this.tileSize,
            this.options.zoom
          );
          let minNumberY = this.callTileNumber(
            leftTopPos.y,
            this.tileSize,
            this.options.zoom
          );
          let maxNumberX = this.callTileNumber(
            rightBottomPos.x,
            this.tileSize,
            this.options.zoom
          );
          let maxNumberY = this.callTileNumber(
            rightBottomPos.y,
            this.tileSize,
            this.options.zoom
          );
        ...
        getTopLevelDomain(url) {
          try {
            ...
            const topLevelDomain = domainParts
              .slice(domainLength - 2)
              .join(".");
        ...
        axiosDownloadImg(z, x, y) {
          ...
          switch (domain) {
            ...
            default: {
              //无法解析底图url则使用高德
              url =
                "http://webrd04.is.autonavi.com/appmaptile?lang=zh_cn&size=1&scale=1&style=8" +
                `&x=${x}&y=${y}&z=${z}`;
            }
          }
            ...
            .then(e => {
              if (this.isTileShouldDownload(z, x, y)) {
                if (e instanceof ImageBitmap) {
                  //判断是否为ImageBitmap
                  const etSize = this.callTileSize(
                    this.tileSize,
                    this.options.zoom
                  );
        ...
        isTileShouldDownload(z, x, y) {
          ...
          return !!this.network.needTiles.find(
            t => z === zoom && t.x === x && t.y === y
          );
        }
        async getImg(vp, z, x, y) {
          const found = this.network.tileCache.find(
            e => e.z === z && e.x === x && e.y === y
          );
          if (found === undefined) {
            //缓存中没有
            const downloadManageObj = this.network.downloadManage.find(
              e => e.z === z && e.x === x && e.y === y
            );
          ...
          return new Promise((resolve, reject) => {
            ...
            downloadObj.xhr.onload = e => {
              if (downloadObj.xhr.status === 200 && downloadObj.xhr.response) {
                let blob = new Blob([downloadObj.xhr.response], {
                  type: "image/png"
                });
                let img = createImageBitmap(blob);
                this.network.tileCache.push({ img: img, z: z, x: x, y: y });
        ...
        viewPositionToLatLng(mouseX, mouseY) {
          //鼠标位置转经纬度
          let mouseVirtualPosition = this.screenToTile(mouseX, mouseY);
          if (
            this.virtualPositionCheck(
              mouseVirtualPosition.x,
              mouseVirtualPosition.y
            )
          ) {
            let lng = this.tileToLng(
              mouseVirtualPosition.x / this.tileSize,
              this.options.zoom
            );
            let lat = this.tileToLat(
              mouseVirtualPosition.y / this.tileSize,
              this.options.zoom
            );
            return { lat, lng };
          } else {
        ...
        virtualPositionCheck(virtualX, virtualY) {
          //检查坐标否处于瓦图范围外
          let maxSize =
            this.options.zoom === parseInt(this.options.zoom)
              ? this.callTileSize(this.tileSize, this.options.zoom) *
                Math.pow(2, this.options.zoom)
              : this.callTileSize(this.tileSize, this.options.zoom) *
                Math.pow(2, parseInt(this.options.zoom) + 1);
          return !(
            virtualX <= 0 ||
            virtualY <= 0 ||
            virtualX >= maxSize ||
        ...
        screenToTile(mouseX, mouseY) {
          //get (screen/Mouse) Position On Tile
          return {
        ...
        optionCheck(baseMapOptions) {
          ...
          if (!this.QIR.hasProperty(baseMapOptions, "resolutionX"))
            return false;
          if (!this.QIR.isNumber(baseMapOptions.resolutionX)) return false;
          if (!this.QIR.hasProperty(baseMapOptions, "resolutionY"))
            return false;
          ...
          if (!this.QIR.hasProperty(baseMapOptions, "defaultZoom"))
            return false;
      ...
      instructPipe: class instructPipe {
        //综合的一个连接服务端的通讯类
        constructor(url) {
          this.url = url;
          ...
          this.Instruct = {
            ...
            broadcast_deleteElement(elementId, tmpId) {
              //以广播的形式删除某一要素
              return {
                type: "broadcast",
                class: "deleteElement",
                data: { id: elementId, tmpId: tmpId }
              };
            },
            ...
            broadcast_updateLayerOrder(passive, active, type) {
              return {
                ...
                data: { passive, active, type }
              };
            },
            broadcast_adjustElementOrder(
              elementA,
              elementB,
              templateA,
              templateB,
              ...
              return {
                ...
                data: { elementA, elementB, templateA, templateB, method }
              };
            },
          ...
          this.QIR = {
            ...
            detailsCheck(details) {
              const ValueExp = /["]/m;
              if (
                Object.prototype.toString.call(details) === "[object Array]"
              ) {
                //1检查是否为数组
                for (let i = 0; i < details.length; i++) {
                  //2循环检查类型
                  if (
                    Object.prototype.toString.call(details[i]) !==
                    "[object object]"
                  ) {
                    //3检查是否为对象
                    if (
                      details[i].hasOwnProperty("key") &&
                      details[i].hasOwnProperty("value")
                    ) {
                      //4检查是否包含key，value属性
                      if (details[i].key === "") {
        ...
        broadcastBatchDeleteElement(id) {
          //批量删除元素
          if (Array.isArray(id)) {
            let len = id.length;
            let lock = false;
            for (let i = 0; i < len; i++) {
              if (typeof id[i] !== "number") lock = true;
            } //检查数组内是否都为元素ID
            if (!lock) {
        ...
        broadcastUpdateLayerOrder(passiveId, activeId, type) {
          this.send(
            this.Instruct.broadcast_updateLayerOrder(passiveId, activeId, type)
          );
        }
        broadcastAdjustElementOrder(
          elementA,
          elementB,
          templateA,
          templateB,
          ...
          this.send(
            this.Instruct.broadcast_adjustElementOrder(
              elementA,
              elementB,
              templateA,
              templateB,
        ...
        broadcastUpdateElementNode(data) {
          ...
          try {
            let sObj = {};
            if (!this.QIR.isObject(data)) {
              return false;
            }
            if (!this.QIR.hasProperty(data, "id")) {
              return false;
            } //1.检查参数是否为数字
            if (!this.QIR.hasProperty(data, "points")) {
              return false;
            }
            if (!this.QIR.isArray(data.points)) {
              return false;
            }
            for (let i = 0; i < data.points.length; i++) {
              let nowCheck = data.points[i];
              if (!this.QIR.isObject(nowCheck)) {
                return false;
              }
              if (
                this.QIR.isNumber(nowCheck.x) === false ||
                this.QIR.isNumber(nowCheck.y) === false
              ) {
                return false;
              }
            }
            ...
            if (this.QIR.hasProperty(data, "point")) {
              if (this.QIR.isObject(data.point)) {
                if (
                  this.QIR.hasProperty(data.point, "x") &&
                  this.QIR.hasProperty(data.point, "y")
                ) {
                  if (
                    this.QIR.isNumber(data.point.x) &&
                    this.QIR.isNumber(data.point.y)
                  ) {
        ...
        broadcastUpdateElement(data, type) {
          //广播更新某一要素
          try {
        ...
        broadcastDeleteElement(id, tmpId) {
          //广播删除某一要素
          if (typeof id !== "number" && typeof tmpId !== "string") {
            return false;
          }
          if (tmpId === "") {
            return false;
        ...
        broadcastRestoreElement(id, tmpId) {
          //广播恢复删除某一要素
          if (typeof id !== "number" && typeof tmpId !== "string") {
            return false;
          }
        ...
        broadcastSendLine(data, Type) {
          //发送路径线数据(兼容区域和曲线类型，只需要在第二个参数Type传入area或curve即可)
          if ("custom" in data) {
            if (
              typeof data.custom.tmpId !== "string" ||
              data.custom.tmpId === ""
            ) {
              this.onLog(
                "添加失败，因为模板id为空，请选中图层后再添加。",
                "error"
              );
              return false;
            }
          ...
          try {
            ...
            if (!this.QIR.hasProperty(data, "class")) {
              return false;
            } //0.2检查是否包含class类型
            if (Type === "area") {
              //0.3检查class类型是否正确
            ...
            if (!this.QIR.hasProperty(data.point, "x")) {
              return false;
            } //0.6检查point是否由xy
            if (!this.QIR.hasProperty(data.point, "y")) {
              return false;
            }
            if (!this.QIR.isNumber(data.point.x)) {
              return false;
            } //0.7检查xy是否是数字
            if (!this.QIR.isNumber(data.point.y)) {
              return false;
            }
            if (!this.QIR.hasProperty(data, "points")) {
              return false;
            ...
            for (let i = 0; i < data.points.length; i++) {
              //0.8.2循环检测内部的xy参数
              if (!this.QIR.hasProperty(data.points[i], "x")) {
                return false;
              } //0.8.3检测是否存在xy值
              if (!this.QIR.hasProperty(data.points[i], "y")) {
                return false;
              }
              if (!this.QIR.isNumber(data.points[i].x)) {
                return false;
              } //0.8.4检测xy是否是数字
              if (!this.QIR.isNumber(data.points[i].y)) {
                return false;
              }
            }
            if (!this.QIR.hasProperty(data, "color")) {
              return false;
            } //0.9检查color
            if (!this.QIR.color16Check(data.color)) {
              return false;
            } //0.10检查颜色
        ...
        instructObjCheck(instructObj) {
          //指令检查
          if (
            Object.prototype.toString.call(instructObj) !== "[object Object]"
          ) {
            return false;
          }
                ...
                try {
                  ...
                  for (let i = 0; i < layerNumber; i++) {
                    if (jsonData.data[i].type === "order") {
                      ...
                      jsonData.data[i].id = parseInt(jsonData.data[i].id); //hack
                      jsonData.data[i].members = JSON.parse(
                        window.atob(jsonData.data[i].members)
                      );
                      jsonData.data[i].structure = JSON.parse(
                        window.atob(jsonData.data[i].structure)
                      );
              ...
              case "broadcast": {
                ...
                switch (classIs) {
                  ...
                  case "line": {
                    //新增线段数据广播
                    try {
                      let [lock, baseA, baseB, Ps, Pt, basePs, basePt] = [
                        true,
                        null,
                        null,
                        null,
                    ...
                    let NewMessageObj = {
                      ...
                      time: jsonData.time,
                      data: { elementId: jsonData.data.id }
                    };
                  ...
                  case "updateElement": {
                    ...
                    try {
                      if (jsonData.data.hasOwnProperty("details")) {
                        //解码details如果有的话
                        jsonData.data.details = JSON.parse(
                          window.atob(jsonData.data.details)
                        );
                      }
                      if (jsonData.data.hasOwnProperty("custom")) {
                        //解码details如果有的话
                        jsonData.data.custom = JSON.parse(
                          window.atob(jsonData.data.custom)
                        );
                      }
                      let eType = undefined;
                      ...
                      if (!found) {
                        this.onLog(
                          "存在未同步的元素,建议刷新页面ID:" + eId,
                          "warn"
                        );
                      }
                    } catch (e) {
                      this.onLog(
                        "存在未同步的元素,建议刷新页面ID:" + jsonData.data.id,
                        "warn"
                      );
                    }
                    break;
                  }
                  case "batchUpdateElement": {
                    //批量更新元素的广播
                    let count = 0;
                    count = jsonData.data.length;
                    for (let K = 0; K < count; K++) {
                      try {
                        if (jsonData.data[K].hasOwnProperty("details")) {
                          //解码details如果有的话
                          jsonData.data[K].details = JSON.parse(
                            window.atob(jsonData.data[K].details)
                          );
                        }
                        if (jsonData.data[K].hasOwnProperty("custom")) {
                          //解码details如果有的话
                          jsonData.data[K].custom = JSON.parse(
                            window.atob(jsonData.data[K].custom)
                          );
                        }
                        let eType = undefined;
                  ...
                  case "updateElementNode": {
                    ...
                    try {
                      //解析
                      let pointsObj = JSON.parse(
                        window.atob(jsonData.data.points)
                      );
                      let basePointsObj = JSON.parse(
                        window.atob(jsonData.data.points)
                      );
                      ...
                      if (this.QIR.hasProperty(jsonData.data, "point")) {
                        ...
                        basePointObj = JSON.parse(
                          window.atob(jsonData.data.point)
                        );
                      }
                  ...
                  case "selectEndElement": {
                    ...
                    for (let i = 0; i < len; i++) {
                      if (this.selectElements[i].id === jsonData.data) {
                        ...
                        let code =
                          parseInt(this.lastSelectEnd.split(":")[1]) + 1;
                  ...
                  case "logIn": {
                    for (let i = 0; i < this.presence.length; i++) {
                      //判断是否重复
                      if (
                        this.presence[i].userEmail === jsonData.data.userEmail
                      ) {
                        return false;
                      }
                    }
                  ...
                  case "createGroupLayer": {
                    if (this.QIR.hasProperty(jsonData, "data")) {
                      if (
                        this.QIR.hasProperty(jsonData.data, "id") &&
                        ...
                        let newMembers = JSON.parse(
                          window.atob(jsonData.data.members)
                        );
                        let newStructure = JSON.parse(
                          window.atob(jsonData.data.structure)
                        );
                        jsonData.data.id = parseInt(jsonData.data.id); //hack
                  ...
                  case "deleteLayerAndMembers": {
                    ...
                    if (changePoint) {
                      //删除点
                      let newPoints = this.mapData.points.filter(element => {
                        return !jsonData.data.members.hasOwnProperty(
                          element.id
                        );
                      });
                    ...
                    if (changeLine) {
                      //删除线段
                      let newLines = this.mapData.lines.filter(element => {
                        return !jsonData.data.members.hasOwnProperty(
                          element.id
                        );
                      });
                    ...
                    if (changeArea) {
                      //删除区域
                      let newAreas = this.mapData.areas.filter(element => {
                        return !jsonData.data.members.hasOwnProperty(
                          element.id
                        );
                      });
                    ...
                    if (changeCurve) {
                      //删除曲线
                      let newCurves = this.mapData.curves.filter(element => {
                        return !jsonData.data.members.hasOwnProperty(
                          element.id
                        );
                      });
                            ...
                            if (newStructure !== undefined) {
                              ...
                              if (
                                ...
                                if (jsonData.data.templateVary === true) {
                                  ...
                                  if (
                                    this.QIR.hasProperty(newCustom, "template")
                                  ) {
                                    let newTmp = newCustom.template;
                                    if (
                                      this.QIR.hasProperty(newTmp, "id") &&
                                      typeof newTmp.id === "string"
                                    ) {
                                      let splitArray = this.lastUpdateTmpId.split(
                                        ":"
                                      );
                                      this.lastUpdateTmpId =
                                        newTmp.id +
                                        ":" +
                  ...
                  case "batchUpdateLayerData": {
                    ...
                    for (let g = 0; g < count; g++) {
                      if (!this.QIR.hasProperty(jsonData.data[g], "id")) {
                        continue;
                      }
                      ...
                      if (this.QIR.hasProperty(jsonData.data[g], "members")) {
                        newMembers = JSON.parse(
                          window.atob(jsonData.data[g].members)
                        );
                      }
                      let newStructure = undefined;
                      if (this.QIR.hasProperty(jsonData.data[g], "structure")) {
                        newStructure = JSON.parse(
                          window.atob(jsonData.data[g].structure)
                        );
                      }
                      for (let i = 0; i < this.mapLayerData.length; i++) {
                        if (ID == this.mapLayerData[i].id) {
                  ...
                  case "renameLayer": {
                    ...
                    for (let i = 0; i < len; i++) {
                      if (
                        this.mapLayerData[i]["id"] === Number(jsonData.data.id)
                      ) {
                        this.mapLayerData[i]["structure"][0] =
                          jsonData.data.name;
      ...
      mixCanvas: class mixCanvas {
        ...
        $manage = {
          optionsRule: {
            //传入options包含的项目及项目的类型\如果required为false则必须包含一个默认值
            viewWidth: {
              //可视宽度
              type: "int",
              required: true
          ...
          colorLinks: {
            //the mapping relationship between color blocks and elements.example'000001':{id:54,type:'point'}
          }
        };
        $iNames = [
          //图标名称
        ...
        mixMove() {
          ...
          this.$canvas.clearRect(
            0,
            0,
            this.pipeline.options.viewWidth,
            this.pipeline.options.viewHeight
          );
          this.$cBlock.clearRect(
            0,
            0,
            this.pipeline.options.viewWidth,
            this.pipeline.options.viewHeight
          ...
          for (let i = 0; i < Len; i++) {
            if (len !== 0) {
              //移动过程中弹出的不渲染
              if (Out.includes(this.$drawCache[i].id)) {
                continue;
              }
            }
            if (LEN !== 0) {
              //移动过程中新增或重新弹入的单独在最后渲染
              if (Join.includes(this.$drawCache[i].id)) {
                continue;
              }
            }
        ...
        mixWash() {
          //清洗canvas和colorBlock，已集成在mixDraw()中
          if (!this.$configs.pipelineHealthy) {
            return false;
          }
          this.$canvas.clearRect(
            0,
        ...
        mixDraw() {
          //绘制元素并同时绘制色块并建立色块与元素的映射关系
          if (this.$moving) return false; //移动过程中不能使用mixDraw()，使用mixMove()替代
          if (
            this.$configs.renderRangeX <= 0 ||
            this.$configs.renderRangeY <= 0
          )
            return false; //当渲染范围为小于等于0时不渲染
          ...
          for (let i = 0; i < aLen; i++) {
            ...
            if (
              this.pipeline.options.mapHiddenElements.has(
                this.pipeline.elements.areas[i].id
              )
            ) {
              continue;
            }
            if (
              this.pipeline.options.mapEjectElements.has(
                this.pipeline.elements.areas[i].id
              )
            ) {
              continue;
            ...
            this.addArea(
              this.pipeline.elements.areas[i],
              colorBlock,
              offsetX,
              offsetY
            );
          ...
          for (let i = 0; i < lLen; i++) {
            ...
            if (
              this.pipeline.options.mapHiddenElements.has(
                this.pipeline.elements.lines[i].id
              )
            ) {
              continue;
            }
            ...
            let colorBlock = cBlockNum.toString(16).padStart(6, "0");
            this.addLine(
              this.pipeline.elements.lines[i],
              colorBlock,
              offsetX,
              offsetY
            );
          ...
          for (let i = 0; i < cLen; i++) {
            ...
            if (
              this.pipeline.options.mapHiddenElements.has(
                this.pipeline.elements.curves[i].id
              )
            ) {
              continue;
            }
            ...
            let colorBlock = cBlockNum.toString(16).padStart(6, "0");
            this.addCurve(
              this.pipeline.elements.curves[i],
              colorBlock,
              offsetX,
              offsetY
            );
          ...
          for (let i = 0; i < pLen; i++) {
            ...
            if (
              this.pipeline.options.mapHiddenElements.has(
                this.pipeline.elements.points[i].id
              )
            ) {
              continue;
            }
            ...
            let colorBlock = cBlockNum.toString(16).padStart(6, "0");
            this.addPoint(
              this.pipeline.elements.points[i],
              colorBlock,
              offsetX,
              offsetY
            );
        ...
        mixColorBlockView() {
          //显示mixColorBlock 调试用途
          let body = document.getElementsByTagName("BODY");
          this.$cBlockDom.setAttribute(
            "style",
            "position:fixed;" +
              "z-index:999999;" +
              "left:0;" +
        ...
        mixReSize() {
          ...
          this.$dom.setAttribute(
            "width",
            this.pipeline.options.viewWidth + "px"
          );
          this.$dom.setAttribute(
            "height",
            this.pipeline.options.viewHeight + "px"
          );
          this.$cBlockDom.setAttribute(
            "width",
            this.pipeline.options.viewWidth + "px"
        ...
        mixSetRenderRange() {
          ...
          if (!isNaN(x) && !isNaN(y)) {
            ...
            this.$configs.renderView.x1 =
              ((1 - this.$configs.renderRangeX) * window.innerWidth) / 2; //x1为渲染边界最左侧
            this.$configs.renderView.x2 =
              this.$configs.renderView.x1 +
              this.$configs.renderRangeX * window.innerWidth;
            this.$configs.renderView.y1 =
              ((1 - this.$configs.renderRangeY) * window.innerHeight) / 2; //y1为渲染边界最上侧
            this.$configs.renderView.y2 =
              this.$configs.renderView.y1 +
              this.$configs.renderRangeY * window.innerHeight;
          } else {
            this.throwError("Error in rendering range parameters");
          }
        }
        mixGetElementByXY(x, y) {
          //通过获取该坐标下的色块颜色,查询渲染此颜色对应的元素是什么(colorLinks)
          if (!this.$configs.pipelineHealthy) return false;
          ...
          if (this.$manage.colorLinks.hasOwnProperty(colorB))
            return this.$manage.colorLinks[colorB];
          return false;
        }
        canvasBuild() {
          //创建基本canvas与虚拟canvas
          ...
          }); //willReadFrequently:true?优化getImageData()的性能
         ...
         **/
        addPoint(element, colorBlock, offsetX, offsetY) {
          //添加点，需要完整的元素数据和色块
          let id = element.id;
          let custom = element.custom;
          let icon; //图标名称
          ...
          if (split) {
            ...
            this.$drawCache.push(
              this.drawCacheObj(id, path, "#" + element.color, "icon", {
                icon,
                XX,
                YY
              })
            ...
            this.$drawCache.push(
              this.drawCacheObj(id, path, "#" + element.color, "point")
            );
          }
        }
        addLine(element, colorBlock, offsetX, offsetY) {
          //添加线，需要完整的元素数据和色块
          let path = new Path2D();
          let id = element.id;
          let Len = element.points.length;
          path.moveTo(
            element.points[0].x + offsetX,
            element.points[0].y + offsetY
          );
          for (let i = 1; i < Len; i++) {
            path.lineTo(
              element.points[i].x + offsetX,
          ...
          this.$cBlock.lineWidth =
            parseInt(element.width, 10) + 1 <= 3
              ? 4
              : parseInt(element.width, 10) + 1; //color block canvas?+1扩大1px渲染范围增大mixGetElementByXY()的容错
        ...
        addArea(element, colorBlock, offsetX, offsetY) {
          //添加区域，需要完整的元素数据和色块
          let path = new Path2D();
          let id = element.id;
          let Len = element.points.length;
          ...
          this.$drawCache.push(
            this.drawCacheObj(id, path, "#" + element.color + "80", "area")
          );
        }
        addCurve(element, colorBlock, offX, offY) {
          //添加曲线，需要完整的元素数据和色块
          let path1 = new Path2D();
          let path2 = new Path2D();
          let id = element.id;
          ...
          this.$cBlock.lineWidth =
            parseInt(element.width, 10) + 1 <= 3
              ? 4
              : parseInt(element.width, 10) + 1; //?+1扩大1px渲染范围增大mixGetElementByXY()的容错
        ...
        addColorLinks(element, colorBlock) {
          //添加色块->元素链接
        ...
        pipelineCheck() {
          if (this.QIR("tc", { value: this.pipeline, correct: "object" })) {
            ...
            if (
              !this.QIR("tc", { value: this.pipeline.el, correct: "string" })
            ) {
            ...
            Object.keys(this.$manage.optionsRule).forEach(
              //检测选项是否正确
              key => {
                if (this.$manage.optionsRule[key].required) {
                  //是否为必须项
                  if (!this.pipeline.options.hasOwnProperty(key)) {
                    ...
                    if (
                      ...
                      //类型错误
                      this.throwError(
                        `option type error:${key} need ${this.$manage.optionsRule[key].type}`
                      );
                      this.pipelineClose();
                      return false;
                    }
                  ...
                  if (!this.pipeline.options.hasOwnProperty(key)) {
                    this.pipeline.options[key] = this.$manage.optionsRule[
                      key
                    ].default;
                  } else {
                    //检查类型
            ...
            if (
              !this.QIR("tc", {
                value: this.pipeline.elements.points,
                correct: "array"
              })
            ) {
            ...
            if (
              !this.QIR("tc", {
                value: this.pipeline.elements.lines,
                correct: "array"
              })
            ) {
            ...
            if (
              !this.QIR("tc", {
                value: this.pipeline.elements.areas,
                correct: "array"
              })
            ) {
      ...
      tmpProof: class tmpProof {
          ...
          switch (oldType) {
            ...
            case "datetime": {
              switch (type) {
                ...
                case "date": {
                  return this.Date(this.datetimeToDate(content));
                }
                case "time": {
                  return this.Time(this.datetimeToTime(content));
                }
                case "list": {
                  return this.List_(content);
                }
                case "percent": {
                  return this.Percent();
                }
            ...
            case "date": {
              switch (type) {
                ...
                case "datetime": {
                  return this.Datetime(this.dateToDatetime(content));
                }
                case "date": {
                  return value;
                } //返回源值
                case "time": {
                  return this.Time(this.dateToTime(content));
                }
                case "list": {
                  return this.List_(content);
                }
            ...
            case "time": {
              switch (type) {
                ...
                case "datetime": {
                  return this.Datetime(this.timeToDatetime());
                }
                case "date": {
                  return this.Date(this.timeToDate());
                }
                case "time": {
                  return value;
                } //返回源值
         ...
         */
        isValidTime(str) {
          //检测字符串是否是时间格式-正确则返回true
          const reg = /^☍m([0-1]?[0-9]|2[0-3]):([0-5]?[0-9])(:([0-5]?[0-9]))?$/;
          return reg.test(str);
        }
        isTime(str) {
          //isValidTime的变体，不检查类型符号
        ...
        isDatetime(str) {
          //isValidDatetime的变体，不检查类型符号
          const reg = /^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])T([01]\d|2[0-3]):([0-5]\d)(:([0-5]\d))?$/;
          return reg.test(str);
        }
        isAllowPercent(value) {
          //检测字符串是否是百分数
          const reg = /^☍p-?\d+(\.\d+)?%$/;
          return reg.test(value);
        }
        isPercent(value) {
          //isAllowPercent的变体，不检查类型符号
          const reg = /^-?\d+(\.\d+)?%$/;
          return reg.test(value);
        }
        isAllowList(value) {
          //检测list字符串是否正确-正确则返回true
          const reg = /^☍l(?!.*,,)(?=.*[^,]$)/;
          return reg.test(value);
        }
        isAllowId(id) {
          //检测模板id是否正确-正确则返回true
          const reg = /^[0-9a-zA-Z]{8,14}$/;
          return reg.test(id);
        }
        isIntegerP(value) {
          //判断一个数字是否为正整数P:positive
          return Number.isInteger(value) && value > 0;
        }
        isNumber(value) {
          //判断一个值是否为数字(不包含无限和NaN)
          if (typeof value !== "number") return false;
          if (isNaN(value)) return false;
          return !(value === -Infinity || value === Infinity);
        }
        isColor16(color) {
          //检测一个字符串是否是标准的16进制颜色-正确则返回true
          const regex = /^#?([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$/;
          return regex.test(color);
        }
        isAllowValueTyp(type, value) {
          //依据type检测value(或default)是否是正确的type类型-正确则返回true
          switch (type) {
            case "text": {
              return typeof value === "string";
            }
        ...
        isAllowMethod(type, method) {
          //判断method是否为type允许使用的方法-正确则返回true
          switch (type) {
            case "number": {
              return [
                "equ",
            ...
            case "percent": {
              return ["equ", "nequ", "gre", "greq", "les", "lesq"].includes(
                method
              );
            }
            case "datetime": {
        ...
        isAllowValueTypL(type, value) {
          //依据type检测value(或default)是否是type类型的数据以及长度是否合理-正确则返回true
          switch (type) {
            case "text": {
              if (typeof value !== "string") return false;
        ...
        codeExplain(code) {
          ...
          switch (B) {
            case 500: {
              return english ? "Template is null" : "模板为空";
            }
            case 1000: {
              return english ? "Template is not an object" : "模板不是一个对象";
            }
            case 2000: {
              return english
                ? "Template missing id attribute"
                : "模板缺失id属性";
            } //A layer property check
            case 2100: {
              return english
                ? "Template id value type error"
                : "模板id值类型错误";
            }
            case 2200: {
              return english
                ? "Template id value cannot be empty"
                : "模板id值不能为空字符";
            }
            case 2300: {
              return english
                ? "Template id does not comply with regulations"
                : "模板id值不符合标准";
            }

            case 4000: {
              return english
                ? "Template missing name attribute"
                : "模板缺失name属性";
            }
            case 4100: {
              return english
                ? "Template name value type error"
                : "模板名称值类型错误";
            }
            case 4200: {
              return english
                ? "Template name value cannot be empty"
                : "模板名称值不能为空字符";
            }

            case 6000: {
              return english
                ? "Template missing creator attribute"
                : "模板缺失creator属性";
            }
            case 6100: {
              return english
                ? "Template creator value type error"
                : "模板创建者值类型错误";
            }
            case 6200: {
              return english
                ? "Template creator value cannot be empty"
                : "模板创建者值不能为空字符";
            }

            case 8000: {
              return english
                ? "Template missing modify attribute"
                : "模板缺少modify属性";
            }
            case 8100: {
              return english
                ? "Template modify value type error"
                : "模板编辑日期值类型错误";
            }
            case 8200: {
              return english
                ? "Template modify value cannot be empty"
                : "模板编辑日期值不能为空";
            }
            case 8300: {
              return english
                ? "Template modify does not comply with regulations"
                : "模板编辑日期值不符合标准";
            }

            case 10000: {
              return english
                ? "Template missing locked attribute"
                : "模板缺失locked属性";
            }
            case 10100: {
              return english
                ? "Template locked value type error"
                : "模板locked值类型错误";
            }

            case 12000: {
              return english
                ? "Template missing explain attribute"
                : "模板缺失explain属性";
            }
            case 12100: {
              return english
                ? "Template explain value type error"
                : "模板描述信息值类型错误";
            }

            case 14000: {
              return english
                ? "Template missing typeRule attribute"
                : "模板缺失typeRule属性";
            } //typeRule property check
            case 15000: {
              return english
                ? "Template typeRule is not an object"
                : "模板typeRule不是一个对象";
            }

            case 16000: {
              return english
                ? "TypeRule missing point attribute"
                : "类型规则缺失point属性";
            }
            case 17000: {
              return english
                ? "TypeRule point value type error"
                : "类型规则point值类型错误";
            }

            case 18000: {
              return english
                ? "TypeRule missing line attribute"
                : "类型规则缺失line属性";
            }
            case 19000: {
              return english
                ? "TypeRule line value type error"
                : "类型规则line值类型错误";
            }

            case 20000: {
              return english
                ? "TypeRule missing area attribute"
                : "类型规则缺失area属性";
            }
            case 21000: {
              return english
                ? "TypeRule area value type error"
                : "类型规则area值类型错误";
            }

            case 22000: {
              return english
                ? "TypeRule missing curve attribute"
                : "类型规则缺失curve属性";
            }
            case 23000: {
              return english
                ? "TypeRule curve value type error"
                : "类型规则curve值类型错误";
            }
            case 24000: {
              return english
                ? "TypeRule at least one must be allowed"
                : "类型规则至少需要允许一个";
            }

            case 30000: {
              return english
                ? "Template missing detailsRule attribute"
                : "模板缺失detailsRule属性";
            } //detailsRule property check
            case 30100: {
              return english
                ? "Template detailsRule is not an array"
                : "模板detailsRule不是一个数组";
            }
            case 30200: {
              return english
                ? "Template detailsRule length cannot = 0"
                : "模板detailsRule长度不能为0";
            }
            case 30300: {
              return english
                ? "Template detailsRule length cannot > 90"
                : "模板detailsRule长度不能大于90";
            }
            case 30400: {
              return english
                ? "Template detailsRule first element type error"
                : "模板detailsRule第一个元素类型错误";
            }
            case 30500: {
              return english
                ? "Template detailsRule first element value error"
                : "模板detailsRule第一个元素值错误";
            }

            case 31000: {
              return english
                ? "Template detailsRule type error in:" + A + " item"
                : "模板属性规则类型错误，在：" + A + "项";
            }
            case 32000: {
              return english
                ? "DetailsRule missing set attribute in:" + A + " item"
                : "属性规则缺失set属性，在：" + A + "项";
            }
            case 32100: {
              return english
                ? "DetailsRule set value type error in:" + A + " item"
                : "属性规则set值类型错误，在：" + A + "项";
            }

            case 33000: {
              return english
                ? "DetailsRule missing name attribute in:" + A + " item"
                : "属性规则缺失name属性，在：" + A + "项";
            }
            case 33100: {
              return english
                ? "DetailsRule name value type error in:" + A + " item"
                : "属性规则name值类型错误，在：" + A + "项";
            }
            case 33200: {
              return english
                ? "DetailsRule name value cannot be empty in:" + A + " item"
                : "属性规则name值不能为空，在：" + A + "项";
            }
            case 33300: {
              return english
                ? "DetailsRule name value length cannot > 40 in:" + A + " item"
                : "属性规则name值长度不能大于40，在：" + A + "项";
            }
            case 33400: {
              return english
                ? "DetailsRule name cannot duplicated:" + A + " item"
                : "属性规则name值不能重复，在：" + A + "项";
            }

            case 35000: {
              return english
                ? "DetailsRule missing type attribute in:" + A + " item"
                : "属性规则缺失type属性，在：" + A + "项";
            }
            case 35100: {
              return english
                ? "DetailsRule type value type error in:" + A + " item"
                : "属性规则type值类型错误，在：" + A + "项";
            }
            case 35200: {
              return english
                ? "DetailsRule type value undefined in:" + A + " item"
                : "属性规则type值未定义，在：" + A + "项";
            }

            case 37000: {
              return english
                ? "DetailsRule missing default attribute in:" + A + " item"
                : "属性规则缺失default属性，在：" + A + "项";
            }
            case 37100: {
              return english
                ? "DetailsRule default value type error in:" + A + " item"
                : "属性规则default值类型错误，在：" + A + "项";
            }

            case 40000: {
              return english
                ? "Template missing colorRule attribute"
                : "模板缺失colorRule属性";
            } //colorRule property check
            case 40100: {
              return english
                ? "Template colorRule is not an object"
                : "模板颜色规则不是一个对象";
            }
            case 40200: {
              return english
                ? "ColorRule missing basis attribute"
                : "颜色规则缺失basis属性";
            }
            case 40300: {
              return english
                ? "ColorRule basis value type error"
                : "颜色规则basis值类型错误";
            }
            case 40400: {
              return english
                ? "ColorRule missing type attribute"
                : "颜色规则缺失type属性";
            }
            case 40500: {
              return english
                ? "ColorRule type value type error"
                : "颜色规则type值类型错误";
            }
            case 40600: {
              return english
                ? "The attribute on which the color rule is based is invalid"
                : "颜色规则所依据的属性无效";
            }
            case 40700: {
              return english
                ? "ColorRule missing condition attribute"
                : "颜色规则缺失condition属性";
            }
            case 40800: {
              return english
                ? "ColorRule condition is not an array"
                : "颜色规则condition不是一个数组";
            }
            case 40900: {
              return english
                ? "If the basis is empty, the type must be empty"
                : "如果basis为空，则type必须为空";
            }
            case 41000: {
              return english
                ? "If the basis is empty, the condition must be an empty array"
                : "如果basis为空，则condition必须是一个空数组";
            }
            case 41100: {
              return english
                ? "If the basis is not empty, the type cannot be empty"
                : "如果basis不为空，则type不能为空";
            }
            case 41200: {
              return english
                ? "ColorRule type value not allowed"
                : "颜色规则type值不允许";
            }
            case 41300: {
              return english
                ? "ColorRule item length cannot > 90"
                : "颜色规则数量不能大于90";
            }

            case 42000: {
              return english
                ? "ColorRule item is not an object in:" + A + " item"
                : "此条颜色规则不是一个对象，在：" + A + "项";
            }
            case 42100: {
              return english
                ? "ColorRule item missing set attribute in:" + A + " item"
                : "此条颜色规则缺失set属性，在：" + A + "项";
            }
            case 42200: {
              return english
                ? "ColorRule item set value type error in:" + A + " item"
                : "此条颜色规则set值类型错误，在：" + A + "项";
            }

            case 43000: {
              return english
                ? "ColorRule item missing color attribute in:" + A + " item"
                : "此条颜色规则缺失color属性，在：" + A + "项";
            }
            case 43100: {
              return english
                ? "ColorRule item color value type error in:" + A + " item"
                : "此条颜色规则color值类型错误，在：" + A + "项";
            }
            case 43200: {
              return english
                ? "ColorRule item color value format error in:" + A + " item"
                : "此条颜色规则color值格式错误，在：" + A + "项";
            }

            case 44000: {
              return english
                ? "ColorRule item missing method attribute in:" + A + " item"
                : "此条颜色规则缺失method属性，在：" + A + "项";
            }
            case 44100: {
              return english
                ? "ColorRule item method value type error in:" + A + " item"
                : "此条颜色规则的method值类型错误，在：" + A + "项";
            }
            case 44200: {
              return english
                ? "ColorRule item method not allowed in:" + A + " item"
                : "此条颜色规则的method不合理，在：" + A + "项";
            }

            case 45000: {
              return english
                ? "ColorRule item missing value attribute in:" + A + " item"
                : "此条颜色规则缺失value属性，在：" + A + "项";
            }
            case 45100: {
              return english
                ? "ColorRule item value type not allowed in:" + A + " item"
                : "此条颜色规则的value类型不合理，在：" + A + "项";
            }
            case 45200: {
              return english
                ? "ColorRule item value length cannot > 100 in:" + A + " item"
                : "此条颜色规则的value字符长度不能大于100，在：" + A + "项";
            }

            case 50000: {
              return english
                ? "Template missing widthRule attribute"
                : "模板缺失widthRule属性";
            } //widthRule property check
            case 50100: {
              return english
                ? "Template widthRule is not an object"
                : "模板widthRule不是一个对象";
            }
            case 50200: {
              return english
                ? "WidthRule missing basis attribute"
                : "宽度规则缺失basis属性";
            }
            case 50300: {
              return english
                ? "WidthRule basis value type error"
                : "宽度规则basis值类型错误";
            }
            case 50400: {
              return english
                ? "WidthRule missing type attribute"
                : "宽度规则缺失type属性";
            }
            case 50500: {
              return english
                ? "WidthRule type value type error"
                : "宽度规则type值类型错误";
            }
            case 50600: {
              return english
                ? "The attribute on which the width rule is based is invalid"
                : "宽度规则所依据的属性无效";
            }
            case 50700: {
              return english
                ? "WidthRule missing condition attribute"
                : "宽度规则缺失condition属性";
            }
            case 50800: {
              return english
                ? "WidthRule condition is not an array"
                : "宽度规则condition不是一个数组";
            }
            case 50900: {
              return english
                ? "If the basis is empty, the type must be empty"
                : "宽度规则中，如果basis为空，则type必须为空";
            }
            case 51000: {
              return english
                ? "If the basis is empty, the condition must be an empty array"
                : "宽度规则中，如果basis为空，则condition必须是一个空数组";
            }
            case 51100: {
              return english
                ? "If the basis is not empty, the type cannot be empty"
                : "宽度规则中，如果basis不为空，则type不能为空";
            }
            case 51200: {
              return english
                ? "WidthRule type value not allowed"
                : "宽度规则type值不合理";
            }
            case 51300: {
              return english
                ? "WidthRule item length cannot > 90"
                : "宽度规则数量不能大于90";
            }

            case 52000: {
              return english
                ? "WidthRule item is not an object in:" + A + " item"
                : "此条宽度规则不是一个对象，在：" + A + "项";
            }
            case 52100: {
              return english
                ? "WidthRule item missing set attribute in:" + A + " item"
                : "此条宽度规则缺失set属性，在：" + A + "项";
            }
            case 52200: {
              return english
                ? "WidthRule item set value type error in:" + A + " item"
                : "此条宽度规则set值类型错误，在：" + A + "项";
            }

            case 53000: {
              return english
                ? "WidthRule item missing width attribute in:" + A + " item"
                : "此条宽度规则缺失width属性，在：" + A + "项";
            }
            case 53100: {
              return english
                ? "WidthRule item width value type error in:" + A + " item"
                : "此条宽度规则width值类型错误，在：" + A + "项";
            }
            case 53200: {
              return english
                ? "WidthRule item width value must be integer in:" + A + " item"
                : "此条宽度规则width值必须为整数，在：" + A + "项";
            }

            case 54000: {
              return english
                ? "WidthRule item missing method attribute in:" + A + " item"
                : "此条宽度规则缺失method属性，在：" + A + "项";
            }
            case 54100: {
              return english
                ? "WidthRule item method value type error in:" + A + " item"
                : "此条宽度规则的method值类型错误，在：" + A + "项";
            }
            case 54200: {
              return english
                ? "WidthRule item method not allowed in:" + A + " item"
                : "此条宽度规则的method不合理，在：" + A + "项";
            }

            case 55000: {
              return english
                ? "WidthRule item missing value attribute in:" + A + " item"
                : "此条宽度规则缺失value属性，在：" + A + "项";
            }
            case 55100: {
              return english
                ? "WidthRule item value type not allowed in:" + A + " item"
                : "此条宽度规则的value类型不合理，在：" + A + "项";
            }
            case 55200: {
              return english
                ? "WidthRule item value length cannot > 100 in:" + A + " item"
                : "此条宽度规则的value字符长度不能大于100，在：" + A + "项";
            }
          }
        }
        /**
        ...
        tpCheck(template) {
          ...
          if (!Object.prototype.hasOwnProperty.call(template, "id"))
            return 2000; //A layer property check
          ...
          if (!Object.prototype.hasOwnProperty.call(template, "detailsRule"))
            return 30000; //detailsRule property check
          ...
          for (let i = 1; i < len; i++) {
            if (!isObj(arr[i])) {
              ...
              if (!Object.prototype.hasOwnProperty.call(arr[i], "set"))
                return 32000 + i + 1;
              if (typeof arr[i].set !== "boolean") return 32100 + i + 1;

              if (!Object.prototype.hasOwnProperty.call(arr[i], "name"))
                return 33000 + i + 1;
              ...
              if (!Object.prototype.hasOwnProperty.call(arr[i], "type"))
                return 35000 + i + 1;
              if (typeof arr[i].type !== "string") return 35100 + i + 1;
              if (!this.isDetailsType(arr[i].type)) return 35200 + i + 1;

              if (!Object.prototype.hasOwnProperty.call(arr[i], "default"))
                return 37000 + i + 1;
              if (!this.isAllowValueTypL(arr[i].type, arr[i].default))
                return 37100 + i + 1;
            }
          }

          if (!Object.prototype.hasOwnProperty.call(template, "colorRule"))
            return 40000; //colorRule property check
          if (!isObj(template.colorRule)) return 40100;
          if (
            !Object.prototype.hasOwnProperty.call(template.colorRule, "basis")
          )
            return 40200;
          if (typeof template.colorRule.basis !== "string") return 40300;
          if (!Object.prototype.hasOwnProperty.call(template.colorRule, "type"))
            return 40400;
          if (typeof template.colorRule.type !== "string") return 40500;
          if (template.colorRule.basis !== "") {
            if (
              !this.isAllowBasis(
                template.colorRule.basis,
                template.colorRule.type,
                template.detailsRule
              )
            )
              return 40600;
          }
          ...
          for (let i = 0; i < len; i++) {
            if (!isObj(arr[i])) {
              ...
              if (!Object.prototype.hasOwnProperty.call(arr[i], "color"))
                return 43000 + i + 1;
              if (typeof arr[i].color !== "string") return 43100 + i + 1;
              if (!this.isColor16(arr[i].color)) return 43200 + i + 1;

              if (!Object.prototype.hasOwnProperty.call(arr[i], "method"))
                return 44000 + i + 1;
              if (typeof arr[i].method !== "string") return 44100 + i + 1;
              if (!this.isAllowMethod(cType, arr[i].method))
                return 44200 + i + 1;

              if (!Object.prototype.hasOwnProperty.call(arr[i], "value"))
                return 45000 + i + 1;
              if (!this.isAllowValueTyp(cType, arr[i].value))
                return 45100 + i + 1;
          ...
          if (!Object.prototype.hasOwnProperty.call(template, "widthRule"))
            return 50000; //widthRule property check
          if (!isObj(template.widthRule)) return 50100;
          if (
            !Object.prototype.hasOwnProperty.call(template.widthRule, "basis")
          )
            return 50200;
          if (typeof template.widthRule.basis !== "string") return 50300;
          if (!Object.prototype.hasOwnProperty.call(template.widthRule, "type"))
            return 50400;
          if (typeof template.widthRule.type !== "string") return 50500;
          if (template.widthRule.basis !== "") {
            if (
              !this.isAllowBasis(
                template.widthRule.basis,
                template.widthRule.type,
                template.detailsRule
              )
            )
              return 50600;
          ...
          for (let i = 0; i < len; i++) {
            if (!isObj(arr[i])) {
              ...
              if (!Object.prototype.hasOwnProperty.call(arr[i], "width"))
                return 53000 + i + 1;
              ...
              if (!this.isAllowMethod(wType, arr[i].method))
                return 54200 + i + 1;

              if (!Object.prototype.hasOwnProperty.call(arr[i], "value"))
                return 55000 + i + 1;
              if (!this.isAllowValueTyp(wType, arr[i].value))
                return 55100 + i + 1;
              if (typeof arr[i].value === "string") {
                //rule(E)
  ...
  getters: {},
  mutations: {
    suppressPickSelect(state, product) {
      //禁用全体元素选中
      state.commits.suppressPickSelect = product;
    },
    clearTempCurveCache(state) {
      //清空临时曲线的缓存
    ...
    arrCoElementPanelHiddenElements(state, product) {
      //product:{type:push|remove|join|quit|byTypeJoin|byTypeQuit,data:Item|Array}
      switch (product.type) {
        case "push": {
          //单一添加
        ...
        case "remove": {
          //单一删除
          let index = state.elementPanelConfig.hiddenElements.findIndex(
            item => {
        ...
        case "join": {
          ...
          state.elementPanelConfig.hiddenElements = merged.filter(
            (value, index, self) =>
        ...
        case "quit": {
          ...
          state.elementPanelConfig.hiddenElements = arr1.filter(
            value => !set.has(value.id)
          );
          break;
        }
        case "byTypeJoin": {
          //按类型集体加入
          let arr1 = state.elementPanelConfig.hiddenElements.filter(
            item => item.type !== product.by
          );
          let arr2 = product.data;
        ...
        case "byTypeQuit": {
          //按类型集体退出
          state.elementPanelConfig.hiddenElements = state.elementPanelConfig.hiddenElements.filter(
            item => item.type !== product.by
          );
          break;
        }
    ...
    setCoLogMessage(state, product) {
      //product:{text:'text',from:'[internal:name ] | [external:name]',type:'warn | error'}
    ...
    setCoLogShowPanel(state, product) {
      //product:true/false
      state.logConfig.showPanel = product;
    },
    setCoToolboxShowPanel(state, product) {
      //product:true/false
      state.toolboxConfig.showPanel = product;
    },
    setCoToolboxPosition(state, product) {
      //product:{x,y}
      state.toolboxConfig.position.x = product.x;
      state.toolboxConfig.position.y = product.y;
    },
    setCoEffectsSvgFlicker(state, product) {
      //product:{svgType:'point'|'line'|'area',svgData:elementObject,duration:millisecond}
    ...
    setCoTemplateShow(state, product) {
      //product:true/false
      state.templateConfig.templateShow = product;
    },
    setCoTemplateEdit(state, product) {
      //product:{ template : {}  , name : 'zoo' , code : number }
      state.templateConfig.editTemplate = JSON.parse(
    ...
    setCoTemplateCancel(state, code) {
      //code (TaskNumber)
    ...
    setCoTemplateSubmit(state, product) {
      //product:{ template : {}  , code : TaskNumber }
    ...
    setCoTemplateUse(state, template) {
      //template:{ ... }
