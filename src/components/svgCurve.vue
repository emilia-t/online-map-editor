    ...
    methods:{
      ...
      initializePosition(){//初始化定位
        if(this.$store.state.baseMapConfig.baseMapType==='realistic'){
          ...
          this.curveConfig.point.x=viewPosition.x;
          this.curveConfig.point.y=viewPosition.y;
          for(let i=0;i<this.dataSourcePoints.length;i++){//循环遍历
            ...
            if(i  in  this.curveConfig.points){
              this.curveConfig.points[i].y=viewPosition.y;
              this.curveConfig.points[i].x=viewPosition.x;
            }else {
