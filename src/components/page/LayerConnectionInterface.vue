...
<div class="connectionInterfaceLayer">
  <div class="InterfaceBox">
    <div class="stripBox">
      <div class="notice">
        <div class="scroll-text-a">
          <span v-text="notice"/>
        </div>
        <div class="scroll-text-b">
          <span v-text="notice"/>
        </div>
      </div>
    ...
    <div class="updOut">
        ...
        <div class="contentItem">
          <div class="contentItemL">
            ...
            <div class="connectionInterfaceBox"><!--连接盒子-->
              <banana-connection-box-default/>
              <banana-connection-box v-for="value in serverLocalConfig" :key="value.serverAddress"
                                     :version="value.version" :account="value.account"
          ...
          <div class="contentItemL">
            <div class="connectionInterfaceBoxT">
              <div class="connectionInterfaceBoxN">
                <split-o-m-r :custom="'margin-right:10px;transform:translateY(1px);'"/>
                路由查询列表
              </div>
            </div>
            <div class="connectionInterfaceBox"><!--连接盒子-->
      ...
      <div class="updRight">
        <div class="updGuide">
          ...
          <div class="updH2">
            欢迎使用OME，我将告诉您如何连接服务器！
          </div>
          <div class="udpText">
            No1. 点击服务器列表右侧的<add-plus-20 :custom="'margin:0px 10px '"/>按钮
          </div>
          <div class="udpText">
            No2. 在弹窗第一行输入服务器的链接
          </div>
          ...
          <div class="udpText">
            PS. 您可以在进入服务器之后再登录或者提前设置账号密码自动进行登录！
          </div>
        </div>
        <div class="updSearch">
          <div class="updTitle">
        ...
        <div class="AncContent"><!--内容-->
          <div class="AncList AncList1"><!--单条目-->
            ...
            <div class="AncInput"><!--输入框-->
              <input ref="UrlInput" placeholder="" type="url" name="url" id="AncUrl" autocomplete="off" @focusin="onFocusMode()" @focusout="noFocusMode()"/>
              <label ref="UrlLabel" for="AncUrl">link</label>
            </div>
          </div>
          <div class="AncList AncList2"><!--单条目-->
            ...
            <div class="AncInput"><!--输入框-->
              <form><input ref="AccountInput" placeholder="" type="text" name="account" id="account" autocomplete="off" @focusin="onFocusMode()" @focusout="noFocusMode()"/></form>
          ...
          <div class="AncList AncList3"><!--单条目-->
            ...
            <div class="AncInput"><!--输入框-->
              <form><input ref="PasswordInput" placeholder="" type="password" name="password" autocomplete="off" maxlength="20" @focusin="onFocusMode()" @focusout="noFocusMode()"/></form>
  ...
  <div class="interfaceBackground" ref="interfaceBackground"/>
</div>
</template>

<script>
import AddPlus from "../svgValidIcons/40X/addPlus";
import AddPlus20 from "../svgValidIcons/addPlus";
...
import BananaConnectionBoxDefault from "./BananaConnectionBoxDefault";
import SplitOMR from "../svgValidIcons/40X/splitOMR";
export default {
  ...
  data(){
    return {
      ...
      notice:'公告：请勿连接不安全的服务器，以避免造成隐私泄露或其他安全性问题。OME已升级至'+this.$root.Version+'版本，请连接对应版本的OMS以避免出现兼容性问题。',
      searchKeyWorld:'',
      searchResult:[],
    }
  },
  components:{
    SplitOMR,
    BananaConnectionBoxDefault,
    EmptyServer,
    BananaConnectionBox,BananaLocalMapBox,Server,AddPlus,AddPlus20
  },
  mounted() {
    this.startSetting();
  },
  methods:{
    ...
    interfaceSearch(){
      ...
      if(serverLocalConfig===false){
        ...
        for(let key in allServerList){//查找是否存在此配置
          let serverName='';
          if('serverName' in allServerList[key]){
            serverName=allServerList[key]['serverName'];
            let regex = new RegExp(this.searchKeyWorld, 'i'); // 'i' 表示不区分大小写
            let status=regex.test(serverName);
            if(status){
              list.push(allServerList[key]);
              find=true;
            }
          }
          let serverKey='';
          if('serverKey' in allServerList[key]){
            serverKey=allServerList[key]['serverKey'];
            let regex = new RegExp(this.searchKeyWorld, 'i'); // 'i' 表示不区分大小写
            let status=regex.test(serverKey);
            if(status){
              list.push(allServerList[key]);
              find=true;
            }
        ...
        if(find===false){
          this.$store.commit('setCoLogMessage',{text:'未找到任何匹配的服务器',from:'internal:LayerConnectionInterface',type:'tip'});
        }
      }
    },
    showBackground(){
      this.$refs.advertisement.classList.toggle("advertisement_hover");
      this.$refs.interfaceBackground.classList.toggle("interfaceBackground_hover");
    },
    autoGetRouteData(){//获取路由数据
      let isEnable=this.$store.state.userSettingConfig.autoGetRoute;
      let useRoute=this.$store.state.userRouteConfig.use;
      if(isEnable!==false && useRoute!==null){
    ...
    createServer(){//创建服务器
      ...
      if(!this.isValidUrl(url)){//2.检测URL合理性
        this.$store.commit('setCoLogMessage',{text:'link格式错误，请参照帮助界面',from:'internal:LayerConnectionInterface',type:'tip'});
        return false;
      }
...
.connectionInterfaceBoxT{
  width: 100%;
  height: 44px;
  box-shadow: 0px 0px 4px rgba(40, 40, 40, 0.27);
  border-radius: 5px;
}
.newLocalMapBoard{
  width: 400px;
...
.scroll-text-a span {
  ...
  padding-left: 100%;;
  animation: scroll-left-a 60s linear infinite;
}
.scroll-text-b{
  white-space: nowrap;
  overflow: hidden;
...
.scroll-text-b span {
  display: inline-block;
  animation: scroll-left-b 60s linear infinite;
}
...
.picture{
  width:calc(100% - 20px);
  height: 100%;
  background: url("/static/background007.png");
...
.advertisement_hover{
  animation: indistinct 8s linear forwards;
  backface-visibility: hidden;
  transform-style: preserve-3d;
}
.interfaceBackground_hover{
  animation: indistinct_background 8s linear forwards;
  backface-visibility: hidden;
}
.interfaceBackground{
  pointer-events: none;
  ...
  width:calc(100%);
  height: 100%;
  background: url("/static/background008.png") no-repeat;
...
@keyframes indistinct_background{
  0% {
    opacity: 0;
  }
  50%{
...
.udpText button{
  ...
  border: none;
  border-radius: 15px;
  background: linear-gradient(to right, #64c9e8, #72eafc);
  color: white;
  font-size: 16px;
  font-weight: bold;
...
.udpText button:hover {
  background: linear-gradient(to right, #56c3e8, #5af0fc);
  box-shadow: 0 6px 12px rgba(0, 0, 0, 0.2);
}

.udpText button:active {
  background: linear-gradient(to right,  #56c3e8, #5af0fc);
