...
<script>
  ...
  export default {
    name: "BananaCurveAttributeBoard",
    components:{OrangeSlideBlock,OrangeColorPalette,PomeloInput},
    data(){
      return {
        id:"-1",
        color:"000000",
    ...
    methods:{
      ...
      submitEdit(){//提交-更新缓存-同时上传数据
        ...
        let obj={//信息汇总
          ...
          class:'curve',//两者一致
          type:'curve',//两者一致
          point:this.tempCurve.point,
          points:this.tempCurve.points,
          color:this.color,
          width:this.width,
        ...
        this.$store.state.serverData.socket.broadcastSendLine(obj,'curve');
        this.$store.commit('clearTempCurveCache');
        this.show=false;
        this.$root.sendInstruct('addNewCurveEnd');
        this.resetDetails();
      },
      cancelEdit(){//取消
        this.width=2;
      ...
      color:{
        handler(newValue,oldValue){
          ...
          if(Exp.test(newValue)===false){
            ...
            this.$store.state.mapConfig.tempCurve.color=newValue;
          }
        }
      },
      width:{
        handler(newValue,oldValue){
          ...
          if(Exp.test(newValue)===false){
            ...
            this.$store.state.mapConfig.tempCurve.width=newValue;
          }
        }
      }
    }
